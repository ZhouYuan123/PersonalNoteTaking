...
unsigned abc;
unsigned int abc;
char ch; // char 在默认情况下既不是没有符号，也不是有符号, 由编译器决定。
for(ch='a'; ch <= 'z'; ch++) // 正确结束
```
...
`int arr[5]`: 创建。array size 不能是变量。如果没有初始化元素，元素的值为以前驻留在该内存单元中的值。

`int arr[] = {1, 2, 3, 4, 5}`: 完全初始化。五个元素。

`int arr[5] = {1, 2, 3, 4, 5}`: 完全初始化。五个元素。
...
++*arr;			   // 第一个元素值+1
(*arr)++           // 先返回第一个元素值然后 再+1
*arr++             // 先返回第一个元素值然后 指针指向第二个元素
arr[i] == *(arr + i);
&arr[i] == arr + i;

arr + 1;		   // 数组中下一个元素地址(返回的数据类型是指针);
arr = arr + 1;     // okay for pointers, wrong for array names. 因为数组名是常量。

int *p1 = arr;     // 因为数组名就是指针
int *p2 = &arr[0]; // 第一个元素取地址。

p1 + 1;            // +1个指针长度. 也是下一个元素
p2 + 1;            // +1个指针长度. 也是下一个元素

int diff = p2 - p1;// 表示之间差了几个元素。

// 例外，数组名表示整个数组
sizeof(arr);       // 返回整个数组的大小，单位字节
sizeof(p1);        // 指针大小。4 or 8
&arr;              // 获取整个数组的地址。 arr，&arr，&arr[0] 三者值相同，但意义不同。
&arr + 1;          // 增加sizeof(数组名)
int (*pas)[5] = &arr; // 数组指针
```
...
**数组 arr 做传参**仅仅传递数组首元素地址(数组长度可写可不写),  所以数组做形参不代表数组。
...
Note: 编译器不会检查使用的下标是否有效。例如，如果将一个值赋给不存在的元素 months[101]，编译器并不会指出错误。但是程序运行后，这种赋值可能引发问题，它可能破坏数据或代码，也可能导致程序异常终止。所以必须确保程序只使用有效的下标值。
...
// 每个值默认从0开始, 一次递增1。可以自定义为相同值。
enum Sex s = MALE; // c++: Sex s = MALE;
s = 1;             // invalid, 1 不是枚举类型
++MALE;            // invalid
int color = MALE;  // valid，MALE转为int类型
s = Sex(1);        // valid, 强制类型转换
```
...
`int * parr[5];` ：存放五个指针。parr先跟5结合，有五个元素，每个元素是 `int *`。
...
//typedef - 对类型进行重定义
typedef void(*pfun_t) (int); // 对void(*) (int)的函数指针类型重命名为pfun_t.  就是typedef void(*) (int) pfun_t,但是语法不支持
...
`__abc, _Abc` : 以两个下划线或下划线和大写字母打头的名称被保留给实现（编译器及其使用的资源）使用。
...
```c
#define BYTE char // 宏，编译程序时用char替换所有的BYTE，从而使BYTE成为char的别名。
...
char arr[] = {'a', 'b' ,'c', '\0'};
char arr[4] = {'a', 'b' ,'c', '\0'};
char arr[] = "abc";       // 隐式最后一个字符是`\0`, 这是字符串的结束标志。
char arr[4] = "abc";
...
// size_t strlen(const char * str); 遇到\0才停止。不包括空字符
...
// 按位比较字符ASCII码值, 返回 <0,0,>0 相等就是返回0
...
| 关键字            |                                                              |
| ----------------- | ------------------------------------------------------------ |
| auto              | 局部变量默认都是用auto修饰的. 自动创建自动销毁。C++11中，被重新定义为用于自动类型推断的关键字，而不再是用于指定变量的存储类别。 |
| extern            | 告知编译器某个变量或函数的定义存在于其他源文件中，当前文件仅作声明，无需重复定义。`#include` 函数默认有 extern, 而变量想共享只能extern。 |
| register          | 建议将该值存放到寄存器中。                                   |
| typedef 原名 新名 | 类型重定义, 例 typedef unsigned int size_t                   |
...
// 我们一般把所有的全局变量和全局函数的实现都放在一个cpp文件里面，然后用一个同名的*.h文件包含所有的函数和变量的声明
...
<img src="../imgs/C/inline.png" style="zoom:100%;" />
...
// 宏的缺点
#define SQUARE(X) X*X
SQUARE(5.0);     // a = 5.0*5.0;
SQUARE(4.5+7.5); // a = 4.5 + 7.5*4.5 + 7.5;
SQUARE(c++);     // a = c++*c++;
```
...
# 汇编 完之后停下来，结果保存在test.o中(elf格式) (windows叫test.obj) 目标代码
gcc -c test.s
# 把汇编代码转换成了机器指令(二进制指令)
# 1. 生成符号表。
