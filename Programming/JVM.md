...
7. PC寄存器用来存储 <font color=blue>**指向下一条指令的地址**</font> 也即将要执行的指令代码。由执行引擎读取下一条指令。
...
**<font color=green> 动态链接: Dynamic Linking (帧数据区)</font>**
...
**<font color=green> 方法返回地址: Return Address (帧数据区)</font>**

一个方法的结束，有两种方式:

* 正常执行完成 （调用该方法的pc寄存器的值）

* 出现未处理的异常，非正常退出 （通过异常表来确定）

**<font color=green> 一些附加信息: (帧数据区)</font>**
...
早期绑定（Early Binding）也被称为静态绑定，它是在编译时完成的。

晚期绑定（Late Binding）也被称为动态绑定，它是在运行时完成的.(对应C++中的虚函数)
...
<font color=blue> 静态类型语言是判断变量自身的类型信息;动态类型语言是判断变量值的类型信息，变量没有类型信息，变量值才有类型信息，这是动态语言的一个重要特征。</font>