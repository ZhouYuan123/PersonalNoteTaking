...
| L | 光标到屏幕底部 |
| { 或者 } | 段落移动。光标移动到空白行 |
| % （可以跟可视模式下使用） | 光标移动到成对的括号并匹配 |
| m + 字母 | 标记当前光标所在位置 |
| ‘ + 字符 | 返回标记光标所在位置 |
| ==== |  |
| v | 可视模式。使用h/j/k/l键可以选中从光标开始的一块。 |
| V | 可视行模式。使用h/j/k/l键可以选中从光标开始的每一行。 |
| ctrl + v | 可视块模式。使用h/j/k/l键进行垂直方向选择。 |
| 数字 + 命令 | 重复一个命令 |
| x , X | 剪切光标所在字符 or nx 多个字符 |
| y + 移动命令                  | 复制移动命令对应的内容 （并不在系统剪切板）                |
| y + w                         | 复制单词                                                   |
| y + $                         | 复制从光标位置到结尾                                       |
| y + ^                         | 复制从光标位置到开头                                       |
...
| A    | 在光标所在行尾插入                                           |
| i    | 在光标所在字符前插入                                         |
| I    | 在光标所在行行首插入（可视块模式只能I进入，例如进行多行注释） |
| o    | 在光标下插入新行                                             |
| O    | 在光标上插入新行                                             |
...
2. 输入i进入编辑模式
3. 输入* 也就是重复的文字
4. 按下ESC 返回到命令模式，返回之后 vi 就会把第23两步的操作重复10次
...
| :noh                                                         | 取消高亮                                                     |
| :s/old/new<br />:s/old/new/g<br />:%s/old/new<br/>:%s/old/new/g<br/>:n1,n2s/old/new/g | 可视区域替换：替换光标所在行第一个<br />可视区域替换：替换光标选中的所有行中的单词<br />全文替行第一个<br/>全文替换字符串<br/>范围内替换。 |
| :%s/old/new/gc                                               | 确认替换，每个都询问。                                       |
...
| :n 文件名                                                    | 新建文件                                                     |
| :w 文件名                                                    | 当前编辑内容写入到新文件                                     |
| :sp [文件名]                                                 | split 横向增加分屏<br />ctrl + w：切换窗口<br />ctrl + w + r: reverse 互换窗口<br />ctrl + w + c: close 关闭窗口<br />ctrl + w + q: quit<br />ctrl + w + o: 关闭其他窗口 |
| :vsp [文件名]                                                | 纵向增加分屏                                                 |
